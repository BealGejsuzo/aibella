...
function getBrowserFingerprint(){
  ...
  try{
    ...
    ctx.fillStyle='#069';ctx.fillText('Aibella\uD83D\uDD12',2,15)
    ctx.fillStyle='rgba(102,204,0,0.7)';ctx.fillText('Aibella\uD83D\uDD12',4,17)
...
function addAssistantActions(wrap,msgEl,ts){
  ...
  shareBtn.addEventListener('click',async()=>{
    const clone=msgEl.cloneNode(true);clone.querySelectorAll('.code-shell,.msg-attachment').forEach(x=>x.remove());const text=(clone.textContent||'').replace(/\s+/g,' ').trim()
    if(navigator.share){
      try{await navigator.share({title:'Aibella',text})}catch(e){}
    }else{
      try{
        await navigator.clipboard.writeText(text)
...
function setUILock(state){
  ...
  if(state==='blocked'||state==='limit'){
    ...
    chatInput.placeholder='Pesan Bella AI'
    const plusBtn=document.getElementById('plusBtn')
...
async function sendCurrentText(){
  ...
  try{
    ...
    updateSendButton()
    let h='Gagal mendapatkan jawaban dari Ai Bella karena masalah database. Silahkan hubungi @Xatanicvxii. '
    h+=navigator.userAgent.includes('Telegram')?'Gagal mendapatkan jawaban dari AI Bella karena koneksi anda terputus. Silahkan berikan perintah ulang. ':'Gagal mendapatkan jawaban dari AI  Bella karena masalah endpoint api. Silahkan hubungi developer. '
